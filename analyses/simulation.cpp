...
void print_usage(const char *prog) {
    fprintf(stderr,
        ...
        "      --help                Show this help\n",
        prog, MAX_SEQ_LEN, READ_LEN, MAX_SKETCH
    );
}
...
void parse_args(int argc, char **argv, params_t *p) {
    static struct option long_opts[] = {
        ...
        {"blend",       no_argument,       0,  1 },
        {"hifi",        no_argument,       0,  2 },
        {"short",       no_argument,       0,  3 },
...
void sketch_blend_sequence(const char *seq, int len, params_t *p, sketch_t *sk) {
    ...
    fuzzy_seeds_len = blend_sketch(seq, len, p->w, p->k, p->blend_bits, p->n_neighbors, 0, &fuzzy_seeds);
