...
void analyze(uint128_t *fuzzy_seeds, uint64_t len,
	...
	if (len > 0) {
		...
		if (BLEND_GET_LENGTH(fuzzy_seeds[0]) < DISTANCE_LENGTH) {
			lengths[BLEND_GET_LENGTH(fuzzy_seeds[0])] += 1;
		} else {
			lengthsXL.push_back(BLEND_GET_LENGTH(fuzzy_seeds[0]));
		}

		for (uint64_t i = 1; i < len; i++) {

			uint32_t current_start = BLEND_GET_INDEX(fuzzy_seeds[i]);
			uint32_t current_end = current_start + BLEND_GET_LENGTH(fuzzy_seeds[i]);
			uint32_t previous_start = BLEND_GET_INDEX(fuzzy_seeds[i-1]);
			uint32_t previous_end = previous_start + BLEND_GET_LENGTH(fuzzy_seeds[i-1]);
...
void process(std::string &sequence,
    ...
    fuzzy_seeds_len = blend_sketch(sequence.c_str(), sequence.length(), window, kmer_size, BLEND_BITS_HIFI, BLEND_NEIGHBOR_NUMBER_HIFI, 0, &fuzzy_seeds);
	...
	analyze(fuzzy_seeds, fuzzy_seeds_len, contiguous_counts, distances, distancesXL, distance_gaps, lengths, lengthsXL, length_gaps, overlap_lengths, overlap_lengthsXL, overlap_length_gaps);

    for (uint64_t index = 0; index < fuzzy_seeds_len; index++) {
        distinct_cores[BLEND_GET_KMER(fuzzy_seeds[index])]++;
    }
...
int main(int argc, char **argv) {
	...
	if (genome.is_open()) {
		...
		while (getline(genome, line)) {
			...
			if (line[0] == '>') {
				...
				if (sequence.size() != 0) {
					...
					if (fuzzy_seeds_len && distinct_cores[BLEND_GET_KMER(fuzzy_seeds[0])] == 1) {
						relaxed_unique++;
					}

					for (uint64_t i = 1; i < fuzzy_seeds_len-1; i++) {
						if (distinct_cores[BLEND_GET_KMER(fuzzy_seeds[i])] == 1 || distinct_cores[BLEND_GET_KMER(fuzzy_seeds[i-1])] == 1 || distinct_cores[BLEND_GET_KMER(fuzzy_seeds[i+1])] == 1) {
							relaxed_unique++;
						}
					}
					if (distinct_cores[BLEND_GET_KMER(fuzzy_seeds[0])] == 1 || distinct_cores[BLEND_GET_KMER(fuzzy_seeds[fuzzy_seeds_len+1])] == 1) {
						relaxed_unique++;
					}
					if (fuzzy_seeds_len) free(fuzzy_seeds);
				}
		...
		if (sequence.size() != 0) {
			...
			for (uint64_t i = 1; i < fuzzy_seeds_len; i++) {
				if (distinct_cores[BLEND_GET_KMER(fuzzy_seeds[i])] == 1 || distinct_cores[BLEND_GET_KMER(fuzzy_seeds[i-1])] == 1) {
					relaxed_unique++;
				}
			}
			if (fuzzy_seeds_len) free(fuzzy_seeds);
