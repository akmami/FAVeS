...
void process(chr_info_t *info, std::string &sequence,
	...
	if (min_seeds_len) {
		info->seeds = min_seeds;
		info->seeds_len = min_seeds_len;
...
int main(int argc, char **argv) {
	...
	if (print_bed) {
		for (int i = 0; i < RELAXATION_SPAN+1; i++) {
			std::string fname = "../out/fa-minimizer-span/min_relaxed_span_" + std::to_string(kmer_size) + "_" + std::to_string(window) + "_" + std::to_string(i) + ".bed";
	...
	// compute unique seeds
	{
		...
		for (uint64_t chrom = 0; chrom < chr_size; chrom++) {
			...
			uint128_t *min_seeds = g_info.chroms->array[chrom]->seeds;
