...
void analyze(uint128_t *min_seeds, uint64_t len,
	...
	if (len > 0) {
		...
		lengths[__minimizer_get_length(min_seeds[0])] += 1;

		for (uint64_t i = 1; i < len; i++) {

			uint64_t previous_start = __minimizer_get_index(min_seeds[i-1]);
			uint64_t previous_end = previous_start + __minimizer_get_length(min_seeds[i-1]);
			uint64_t current_start = __minimizer_get_index(min_seeds[i]);
			uint64_t current_end = current_start + __minimizer_get_length(min_seeds[i]);
...
void process(chr_info_t *info, std::string &sequence,
    ...
    min_seeds_len = sketch_minimizers(sequence.c_str(), sequence.length(), window, kmer_size, 1, chr_idx, &min_seeds);
	...
	core_counts += min_seeds_len;
    sizes += sizeof(uint128_t) * min_seeds_len;
	
	analyze(min_seeds, min_seeds_len, contiguous_counts, lengths, length_gaps);

    for (uint64_t index = 0; index < min_seeds_len; index++) {
        distinct_cores[__minimizer_get_kmer(min_seeds[index])]++;
    }

	sequence.clear();

	if (min_seeds_len) {
		info->min_seeds = min_seeds;
		info->min_seeds_len = min_seeds_len;
		info->is_unique = (uint8_t *)calloc(min_seeds_len, sizeof(uint8_t));
	} else {
		info->min_seeds = NULL;
		info->min_seeds_len = 0;
		info->is_unique = NULL;
	}
}
...
void relaxed_uniqueness_paint(chr_info_t *info,
	...
	uint128_t *min_seeds = info->min_seeds;
	uint64_t min_seeds_len = info->min_seeds_len;
	const uint8_t *is_unique = info->is_unique;

    for (int radius = 0; radius <= RELAXATION_SPAN; radius++) {
        uint8_t *relaxed = (uint8_t*)calloc(min_seeds_len, 1);
        if (!relaxed) abort();

        for (uint64_t seed_index = 0; seed_index < min_seeds_len; seed_index++) {
            if (!is_unique[seed_index]) continue;
			
            int64_t left_index = clamp_i64((int64_t)seed_index - radius, 0, (int64_t)min_seeds_len - 1);
            int64_t right_index = clamp_i64((int64_t)seed_index + radius, 0, (int64_t)min_seeds_len - 1);
		...
		for (uint64_t seed_index= 0; seed_index < min_seeds_len; seed_index++) {
			...
			uint64_t s = __minimizer_get_index(min_seeds[seed_index]);
			uint64_t e = s + __minimizer_get_length(min_seeds[seed_index]);
...
int main(int argc, char **argv) {
	...
	if (print_bed) {
		for (int i = 0; i < RELAXATION_SPAN+1; i++) {
			std::string fname = "../out/min_relaxed_span_" + std::to_string(i) + ".bed";
		...
		while (getline(genome, line)) {
			...
			if (line[0] == '>') {
				...
				if (sequence.size() != 0) {
					...
					process(info, sequence, window, kmer_size,
	...
	// compute unique seeds
	{
		...
		for (uint64_t chrom = 0; chrom < chr_size; chrom++) {
			if (g_info.chroms->array[chrom]->min_seeds == NULL) continue;
			if (g_info.chroms->array[chrom]->is_unique == NULL) { fprintf(stderr, "should not happend\n"); continue; }

			// good for readability
			uint128_t *min_seeds = g_info.chroms->array[chrom]->min_seeds;
			uint64_t seed_len = g_info.chroms->array[chrom]->min_seeds_len;
			uint8_t *is_unique = g_info.chroms->array[chrom]->is_unique;
			
			for (uint64_t seed_index = 0; seed_index < seed_len; seed_index++) {
				auto it = distinct_cores.find(__minimizer_get_kmer(min_seeds[seed_index]));
	...
	for (uint64_t chrom = 0; chrom < g_info.chroms->size; chrom++) {
		if (g_info.chroms->array[chrom]) {
			if (g_info.chroms->array[chrom]->min_seeds) free(g_info.chroms->array[chrom]->min_seeds);
